...
#list-table td, #list-table th { text-align : center; }
	...
	<div class="container">
		...
		<div class="container my-5">
					...
					<c:choose>
						...
						<c:otherwise> <!-- 게시글이 있을 때 모두 출력-->
							<c:forEach var="mypage" items="${pList}">
								<tr>
									<td class="mypageNo">${mypage.brdNo}</td>
									<td class="typeTd">${mypage.brdType}</td>
									<td class="mypageTitle" style="text-align : left;">
										${mypage.brdTitle}
									</td>
									<td class="mypageContent" style="text-align : left;">${mypage.replyContent}</td>
									<td> 
									<!-- 날짜 출력 모양 지정 변수 선언 -->
										<!-- *조건 확인용 오늘 날짜 -->
			...
			<%-- boardList.jsp에서 복붙한 거 놔두면 오류나서 메모장 파일에 옮겨놓음 --%>
		 	<c:url var="pageUrl" value="member/myActiveReply.do"/>
	...
	<script>
		...
		$("#list-table td").on("click", function(){
			...
			var url = "${contextPath}/" + brdType + "/view.do?tp=" + type + 
					  "&from=myR&cp=${pInfo.currentPage}&no=" + searchBrdNo; 
